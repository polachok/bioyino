        ...
        let renew_loop = loop_fn((), move |()| {
            ...
            let renew =
                ...
                loop_session.and_then(move |sid| {
                    ...
                    timer.map_err(ConsulError::Io).for_each(move |_| {
...
impl IntoFuture for ConsulSession {
    ...
    fn into_future(self) -> Self::Future {
        ...
        let ttl_ns = ttl.as_secs() * 1_000_000_000u64 + u64::from(ttl.subsec_nanos());
        let b = format!(
            "{{\"TTL\": \"{}ns\", \"LockDelay\": \"{}ns\"}}",
            ttl_ns, ttl_ns
        );
